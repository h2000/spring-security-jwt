...
public class JWKProvider {
  ...
  public JWKProvider() {
    ...
    try {
      privateKey =
          (RSAPrivateKey)
              PemUtils.readPrivateKey(getClass().getResourceAsStream("rsa-private.pem"), "RSA");
      publicKey =
          (RSAPublicKey)
              PemUtils.readPublicKey(getClass().getResourceAsStream("rsa-public.pem"), "RSA");
    } catch (IOException e) {
  ...
  public Jwk create(String keyId) {
    ...
    additionalValues.put(
        "e", Base64.encodeBase64String(publicKey.getPublicExponent().toByteArray()));
